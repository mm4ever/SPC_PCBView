...
public:
  ...
  {
//      this->len = len;
//      this->ptr = new uint8_t[this->len];
//      memcpy(const_cast<uint8_t*>(this->ptr),ptr,this->len);
  }
//  //rime 170526 这是一个指针, 所以在赋值的时候需要考虑深复制
//  //这个类源代码居然用了浅复制,总觉得有内存泄露的风险, 使用valgrint暂时没有发现泄露, 后续再注意
//  ByteArray(const ByteArray& array)
//  {
//      this->len = array.len;
//      this->ptr = new uint8_t[this->len];
//      memcpy(const_cast<uint8_t*>(this->ptr),array.ptr,this->len);
//  }
//  ~ByteArray()
//  {
//      if(0 !=this->len)
